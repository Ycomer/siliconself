...
export default function App() {
  ...
  return (
    <>
      ...
      <Vision />
      <CTA />
      <section id="privacy" className="privacy-copy"><h2>{zh ? '你的记录，由你掌握。' : 'Your journal stays with you.'}</h2><p>{zh ? '自助实验的日记只存储于当前浏览器，不上传、不用于训练，也不会通过邮件发送。共享设备上的其他使用者可能看到本地记录。你可以随时导出或清除。' : 'Self-guided journal entries stay in this browser. They are not uploaded, used for training or emailed. Other people using the same browser may access local entries. You can export or clear them at any time.'}</p><p>{zh ? '内测表单只收集邮箱、兴趣、可选目标、联系许可与来源渠道，保存于 Cloudflare D1，并通过 Cloudflare 邮件服务通知 Fankus 的已验证收件邮箱。只按你选择的兴趣跟进，不出售资料。日记内容和邮箱不会发送到访问统计。' : 'The pilot form collects your email, focus, optional goal, consent and referral source. Requests are stored in Cloudflare D1 and sent through Cloudflare Email to Fankus’s verified inbox. Follow-up is limited to your selected interest; data is not sold. Journal text and email addresses are not sent to analytics.'}</p><p>{zh ? '如需退订、导出或删除申请，请写信至' : 'For unsubscribe, export or deletion requests, contact'} <a href="mailto:hello@siliconself.xyz">hello@siliconself.xyz</a>。{zh ? '陪伴内测仍在招募；AI 分析与付费方案尚未开放。' : 'The guided pilot is recruiting. AI analysis and paid plans are not yet available.'}</p></section>
      <Footer />
    </>
  );
}
